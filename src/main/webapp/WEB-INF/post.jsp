    ...
    <div class="container">
        <div class="row bs-callout bs-callout-warning">
            <div class="col col-md-1 col-sm-2">
                <c:choose>
                    <c:when test="${requestScope.post.postVoteType eq 'POSITIVE'}">
                        <a href="${pageContext.request.contextPath}/vote?post_id=${requestScope.post.postId}&is_positive=true&post-page=true&page=${requestScope.pageNumber}" class="btn btn-block btn-primary btn-outline-success my-bt-vote-up"><span class="glyphicon glyphicon-thumbs-up"></span>  </a>
                    </c:when>
                    <c:otherwise>
                        <a href="${pageContext.request.contextPath}/vote?post_id=${requestScope.post.postId}&is_positive=true&post-page=true&page=${requestScope.pageNumber}" class="btn btn-block btn-primary btn-outline-success btn-success"><span class="glyphicon glyphicon-thumbs-up"></span>  </a>
                    </c:otherwise>
                </c:choose>
                ...
                <c:choose>
                    <c:when test="${requestScope.post.postVoteType eq 'NEGATIVE'}">
                        <a href="${pageContext.request.contextPath}/vote?post_id=${requestScope.post.postId}&is_positive=false&post-page=true&page=${requestScope.pageNumber}" class="btn btn-block btn-primary btn-outline-danger my-bt-vote-down"><span class="glyphicon glyphicon-thumbs-down"></span>  </a>
                    </c:when>
                    <c:otherwise>
                        <a href="${pageContext.request.contextPath}/vote?post_id=${requestScope.post.postId}&is_positive=false&post-page=true&page=${requestScope.pageNumber}" class="btn btn-block btn-primary btn-outline-danger btn-danger"><span class="glyphicon glyphicon-thumbs-down"></span>  </a>
                    </c:otherwise>
                </c:choose>
            </div>
                    ...
                    <c:when test="${requestScope.isEditing != null}">
                        <form class="form-signin" method="post" action="edit-post?post-id=${requestScope.post.postId}">
                            ...
                            <textarea name="inputDescription" rows="5" class="form-control"
                                      placeholder="post description - max 100 character (more will be cut)" required autofocus>${requestScope.post.description}</textarea>
                            <textarea name="inputMessage" rows="5" class="form-control"
                                       placeholder="post message - max 500 character (more will be cut)" required autofocus>${requestScope.post.message}</textarea>
