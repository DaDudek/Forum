                ...
                <div class="col col-md-1 col-sm-2">
                    <c:choose>
                        <c:when test="${post.postVoteType eq 'POSITIVE'}">
                            <a href="${pageContext.request.contextPath}/vote?post_id=${post.postId}&is_positive=true" class="btn btn-block btn-primary my-bt-vote-up"><span class="glyphicon glyphicon-thumbs-up"></span>  </a>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${post.postVoteType eq 'NEGATIVE'}">
                            <a href="${pageContext.request.contextPath}/vote?post_id=${post.postId}&is_positive=false" class="btn btn-block btn-primary my-bt-vote-down"><span class="glyphicon glyphicon-thumbs-down"></span>  </a>
                        </c:when>
                        <c:otherwise>
    ...
    <nav aria-label="...">
        <ul class="pagination">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item">
                        <a class="page-link" href="${pageContext.request.contextPath}/search?sort=${requestScope.postSort}&keywords=${requestScope.keywords}&page=${requestScope.pageNumber-1}">Previous</a>
                    </li>
                </c:otherwise>
            </c:choose>
            ...
            <c:forEach var="page" items="${requestScope.pages}">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item">
                            <a class="page-link" href="${pageContext.request.contextPath}/search?sort=${requestScope.postSort}&keywords=${requestScope.keywords}&page=${page.toString()}"><c:out value="${page.toString()}" /></a>
                        </li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item">
                        <a class="page-link" href="${pageContext.request.contextPath}/search?sort=${requestScope.postSort}&keywords=${requestScope.keywords}&page=${requestScope.pageNumber+1}">Next</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </ul>
